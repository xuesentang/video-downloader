  ...
  >
    ...
    <div class="relative max-w-4xl mx-auto px-4 sm:px-6 text-center">
      ...
      <template v-if="showSlogan">
        <div 
          class="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-white shadow-sm border border-border-light text-sm text-text-secondary"
        ...
        <h1 
          ...
          class="font-bold text-text-primary leading-tight"
        >
          免费在线视频下载器
        ...
        >
          粘贴视频链接，智能解析下载。支持 YouTube、Bilibili、抖音、TikTok 等 1800+ 平台，多种清晰度可选
        </p>
      </template>

      <!-- 搜索输入框 -->
      <div class="max-w-2xl mx-auto">
        <form @submit.prevent="onSubmit" class="relative flex items-center" role="search">
          ...
          <button
            ...
            class="hidden sm:flex items-center gap-2 h-14 px-8 rounded-r-full bg-primary hover:bg-primary-dark text-white font-medium text-base transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-md hover:shadow-lg"
          >
        ...
        <div v-if="showSlogan" class="flex flex-wrap items-center justify-center gap-3 mt-5 text-xs text-text-muted">
          ...
          <button
            ...
            class="px-3 py-1 rounded-full bg-white border border-border-light hover:border-primary hover:text-primary transition-all"
          >
            {{ example.label }}
          </button>
        </div>
 ...
 * Props:
 *   @loading - 是否正在解析中
 *   @compact - 是否紧凑模式（解析成功后）
 *   @showSlogan - 是否显示标题和副标题
 * 
 * Events:
 *   @parse(url) - 点击解析按钮时触发，参数为输入的 URL
 */
